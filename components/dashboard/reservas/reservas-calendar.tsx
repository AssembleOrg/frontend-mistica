...
export function ReservasCalendar({
  ...
  anchor?: string;
  /** Oculta la barra de mes propia (cuando el padre ya tiene la suya). */
  hideHeader?: boolean;
}) {
  const now = new Date();
  const [ownYear, setYear] = useState(() =>
    Number(
      new Intl.DateTimeFormat('en-CA', { timeZone: AR_TZ, year: 'numeric' }).format(now),
    ),
  );
  const [ownMonth, setMonth] = useState(() =>
    Number(
      new Intl.DateTimeFormat('en-CA', { timeZone: AR_TZ, month: '2-digit' }).format(now),
    ) - 1,
  );
  const year = anchor ? Number(anchor.slice(0, 4)) : ownYear;
  const month = anchor ? Number(anchor.slice(5, 7)) - 1 : ownMonth;
  const [items, setItems] = useState<ReservationItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [selected, setSelected] = useState<string>(() => anchor ?? todayYmd());

  // Modo controlado: al mover el mes desde afuera, el día elegido (mobile)
  // acompaña.
  useEffect(() => {
